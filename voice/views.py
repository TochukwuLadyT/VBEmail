...
attachment_dir = 'C:/Users/Anikings/Desktop/Attachment/'
...
def convert_special_char(text):
    temp=text
    special_chars = ['dot','underscore','dollar','hash','star','plus','minus','space','dash','at','exclamation']
    for character in special_chars:
        while(True):
            ...
            if pos == -1:
                ...
                elif character == 'exclamation':
                    temp=temp.replace('exclamation','!')
    return temp
...
def login_view(request):
    ...
    if request.method == 'POST':
        text1 = "Welcome to Voice Based Email Portal. Login with your email account to continue. "
        ...
        imap_url = 'imap.gmail.com'
        #addr = 'exercisetravel@gmail.com'
        #passwrd = 'exercise+*'
        conn = imaplib.IMAP4_SSL(imap_url)
        try:
            conn.login(addr, passwrd)
            s.login(addr, passwrd)
            texttospeech("Congratulations. You have logged in successfully. You will now be redirected to options page.", file + i)
            i = i + str(1)
            return JsonResponse({'result': 'success'})
            #return redirect('homepage/options.html')
        except:
...
def options_view(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'voice/options.html')
...
def compose_view(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            texttospeech("Your email has been sent successfully. You will now be redirected to the options page.", file + i)
            i = i + str(1)
        except:
        ...
        s.quit()
        return JsonResponse({'result' : 'success'})
    return render(request, 'voice/compose.html')
    ...
    for part in msg.walk():
        ...
        if bool(filename):
            ...
            with open(filepath, "wb") as f:
                ...
                i = i + str(1)
                path = 'C:/Users/Anikings/Desktop/Attachment'
        ...
        while flag1:
            while True:
                ...
                if yn == 'yes':
                    ...
                    newtoaddr.append(to)
                    break
            texttospeech("Do you want to add more recipients?", file + i)
            i = i + str(1)
            ans1 = speechtotext(2)
            ans1 = ans1.lower()
            print(ans1)
...
def inbox_view(request):
    ...
    if request.method == 'POST':
        ...
        text = "You have reached your inbox. There are " + str(len(mail_list)) + " total mails in your inbox. You have " + str(no) + " unread emails" + ". To read unread emails say unread. To search a specific email say search. To go back to the options page say back. To logout say logout."
        ...
        while(flag):
            ...
            if act == 'n':
                ...
                texttospeech("You will now be redirected to the options page.", file + i)
                i = i + str(1)
                conn.logout()
                return JsonResponse({'result': 'success'})
        ...
        return render(request, 'voice/inbox.html')
...
def sent_view(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'voice/sent.html')
